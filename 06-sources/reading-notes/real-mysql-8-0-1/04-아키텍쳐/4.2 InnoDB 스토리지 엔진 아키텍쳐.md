...
> [!abstract] 이 절의 한 문장
> InnoDB는 clustered index와 MVCC·lock으로 row를 관리하고, buffer pool·redo·page cleaner로 읽기 cache와 지연 쓰기를 구현한다. Doublewrite는 torn page를 복구하고, undo log는 rollback과 consistent read에 필요한 이전 version을 보존한다. Change buffer는 secondary index random I/O를 merge 시점으로 미룬다. Redo log는 WAL로 commit의 지속성과 crash recovery를 보장한다. Adaptive Hash Index는 buffer pool의 반복적인 B+Tree 검색 pattern을 관찰해 hot page로 가는 hash shortcut을 자동 생성하지만, memory·유지 비용과 latch 경합 때문에 실제 workload를 A/B 측정해 사용 여부를 정한다.
...
flowchart LR
    ...
    SI -->|"PK로 재탐색"| CI
    SI -->|"buffer pool miss + non-unique 등<br/>buffering 가능 조건"| CB["change buffer<br/>secondary index 변경 조각"]
    CB -->|"page read·background·slow shutdown에서 merge"| BP
    CB --> SYS["system tablespace<br/>재시작 뒤에도 merge 가능한 영속 정보"]
    INNO --> FK["foreign key 검사<br/>parent·child index"]
    FK --> LOCK["record·gap·next-key lock"]
    INNO --> BP["buffer pool<br/>현재 data·index page"]
    INNO --> OBS["반복되는 B+Tree 검색 pattern 관찰"]
    OBS --> AHI["adaptive hash index<br/>(index ID + key prefix) → hot page"]
    AHI -->|"root·branch 탐색 우회"| BP
    AHI --> AHIL["partition latch<br/>hotspot이면 contention 가능"]
    INNO --> UNDO["undo log<br/>이전 row version"]
    UNDO --> RSEG["rollback segment<br/>undo slots"]
    RSEG --> UTS["undo tablespace<br/>undo_001·undo_002"]
    UNDO --> HIST["history list<br/>purge 대기 undo"]
    HIST --> PURGE["purge threads<br/>불필요한 version 정리"]
    INNO --> LB["log buffer<br/>memory의 redo record"]
    LB -->|"commit flush policy"| REDO["redo log<br/>변경의 재실행 정보"]
    REDO --> ACK["COMMIT 성공 응답<br/>기본 1은 fsync 후"]
    ...
    LONG["long transaction<br/>old read view"] -->|"purge horizon을 붙잡음"| HIST
    BP --> LIST["LRU·free·flush list<br/>page 수명과 dirty page 관리"]
    REDO --> CKPT["checkpoint LSN<br/>재사용 가능한 redo 경계"]
    REDO --> ARCH["redo archive<br/>물리 backup이 필요한 redo 보존"]
...
사진의 `buffer pool·change buffer·adaptive hash index·log buffer·background thread·data file·transaction log`은 4.2 전체에서 하나씩 확장한다. 현재까지의 중심은 **row와 version 선택**, **lock wait cycle**, **crash recovery**, **memory page와 redo 공간의 순환**, **flush 속도 제어**, **재시작 warmup**, **torn page 방어**, **undo의 생성·보존·purge·truncate 생명주기**, **secondary index random I/O의 지연과 merge 비용**, **반복 B+Tree 탐색을 줄이는 hash shortcut과 contention trade-off**다.
...
```text
4.2.9 언두 로그
InnoDB 스토리지 엔진은 트랜잭션과 격리 수준을 보장하기 위해 DML로 변경되기 이전 버전의 데이터를 별도 백업한다.

언두로그는 
- 트랜잭션 보장
  - 트랜잭션이 롤백될 때 이전 데이터로 복구하기 위해 사용된다
- 격리 수준 보장
  - 특정 커넥션에서 데이터를 변경하는 도중 다른 커넥션에서 데이터를 조회하면 트랜잭션 격리 수준에 맞게 언두 로그에서 데이터를 읽어 반환하기도 한다.

4.2.9.1 언두 로그 모니터링
언두 로그 용량이 급증하는 상황
- 100GB의 데이터를 삭제하면 언두로그에도 100GB의 데이터가 쌓인다
- 트랜잭션을 오랜 시간 실행해도 급증할 수 있다.
트랜잭션이 완료됐다고해서 해당 트랜잭션이 생성한 언두 로그를 즉시 삭제할 수 있는건 아니다.
->  한 트랜잭션이 시작되고 끝나기 전에 다른 트랜잭션이 끝났다면 다른 트랜잭션들의 변경사항도 트랜잭션A가 끝나기 전까지 언두로그에 유지해야함

4.2.9.2 언두 테이블스페이스 관리
언두 로그가 저장되는 공간을 언두 테이블스페이스(Undo Tablespace)라고한다.
...
```text
4.2.10 체인지 버퍼
인덱스 업데이트 작업은 랜덤 디스크 읽기 작업이 필요하므로 테이블에 인덱스가 많다면 많은 자원을 소모함

그래서 InnoDB는 변경해야할 인덱스 페이지가 버퍼 풀에 있으면 바로 업데이트를 수행하지만, 없는 경우 즉시 디스크에서 읽지 않고 임시 공간에 저장해 둔 후 사용자에게 결과를 반환하는 형태로 성능을 향상시키는데 이때 사용하는 임시 메모리 공간을 체인지 버퍼라고한다.

유닉스 인덱스(결과 전달 전 중복 여부를 체크해야함/뭔지 적어두기)는 체인지 버퍼 사용 x

체인지 버퍼 머지 스레드 : 체인지 버퍼에 임시로 저장된 인덱스 레코드 조각이 백그라운드 스레드에 의해 병합되는 것
...
### 2026-08-14 · 4.2.11 리두 로그 및 로그 버퍼
...
리두 로그는 ACID 중 D와 밀접한 관계가 있다. 리두 로그는 MySQL 서버가 비정상 적으로 종료되었을 때 데이터 파일에 기록되지 못한 데이터를 잃지 않게 해주는 안전 장치

대부분의 데이터베이스 서버는 데이터 변경 내용을 로그로 먼저 기록


이제 내가 사진으로 제공할 때는 밑줄을 내가 보낸 글이라고 판단하고 작업해줘. 그 외에도 중요한 부분은 명확히 기록해줘
```

#### [사진 표시 전사]

```text
- data file은 쓰기보다 읽기에 유리한 구조이므로 변경된 data page를 바로 쓰면 random I/O 비용이 크다. 쓰기 비용이 낮은 redo log를 먼저 기록하고, crash 후 redo로 data file을 장애 직전 상태로 복구한다.
- 서버가 crash하면 (1) COMMIT됐지만 data file에 아직 기록되지 않은 data, (2) ROLLBACK됐지만 data file에 이미 기록된 data가 있을 수 있다.
- 1은 redo로 다시 적용하고, 2는 undo로 변경 전 data를 복구한다. redo는 transaction이 commit·rollback·실행 중이었는지 판단하는 데도 필요하다.
- redo는 COMMIT 시점에 disk에 기록되도록 설정하는 것이 안전하지만, 매 commit마다 disk sync를 하면 부하가 크다. `innodb_flush_log_at_trx_commit`으로 주기를 결정한다.
- `innodb_flush_log_at_trx_commit=0`이나 `2`는 crash 시 최대 약 1초 정도의 transaction data가 유실될 수 있다. DDL 등으로 더 자주 flush될 수도 있다.
- redo 용량은 buffer pool의 dirty page 변경량을 모았다가 disk에 batch로 기록할 수 있는 여유와 연결된다. 변경량이 많을수록 redo write가 병목이 될 수 있으며 ACID를 보장하는 범위에서 buffering한다.
- log buffer는 redo를 memory에 buffering하는 공간이다. 기본 16MB 수준이며 BLOB·TEXT처럼 큰 data를 자주 변경하는 workload는 더 큰 값을 검토한다.
- [여백 질문] redo log는 왜 archive할까?
- redo log를 비활성화한 bulk load는 crash 시 복구할 수 없고 data 불일치를 만들 수 있으므로, 완료 후 반드시 다시 활성화한다.
```

</details>

### 2026-08-14 · 4.2.12 어댑티브 해시 인덱스
...
```text
- Adaptive Hash Index는 사용자가 직접 table에 생성하는 index가 아니라 InnoDB가 자주 요청되는 data에 대해 자동으로 생성하는 index다. `innodb_adaptive_hash_index`로 활성화·비활성화할 수 있다.
- B-Tree에서 특정 값을 찾으려면 root node부터 branch node를 거쳐 leaf node까지 찾아가야 하며, 동시 query가 많아지면 CPU가 많은 일을 하게 된다.
- Adaptive Hash Index는 자주 읽히는 data page의 key 값으로 hash index를 만들고, 다음부터 B-Tree root부터 leaf까지 내려가는 비용 없이 record가 저장된 data page를 즉시 찾게 한다.
- hash key에는 B-Tree index의 고유번호와 실제 key 값이 포함된다. InnoDB의 AHI는 하나지만 partition할 수 있다.
- AHI는 memory에 있는 page 접근을 빠르게 할 뿐 disk read를 줄이지 못한다. disk data가 buffer pool보다 크거나, JOIN·LIKE pattern 검색이 많거나, 넓은 범위를 읽으면 이점이 작을 수 있다.
- disk data가 buffer pool과 비슷해 disk read가 많지 않고, equality·IN 검색이 많으며, query가 일부 hot data에 집중되면 도움이 될 수 있다.
- AHI가 많은 memory를 사용하거나 효율이 낮으면 hash lookup과 B-Tree lookup을 모두 수행하는 비용이 생길 수 있다. table DROP·ALTER 때 관련 entry 제거 비용도 고려한다.
- `SHOW ENGINE INNODB STATUS`의 `hash searches/s`와 `non-hash searches/s`를 비교해 사용 정도를 확인할 수 있다.
- `performance_schema.memory_summary_global_by_event_name`의 `memory/innodb/adaptive hash index`로 계측된 memory 사용량을 확인할 수 있다.
- [여백 질문] Spring JPA에서 어떻게?
```
...
> [!summary] 30초 요약
> undo log는 DML 이전 상태로 되돌리는 데 필요한 기록이다. **rollback** 때는 변경을 취소하고, **MVCC consistent read** 때는 read view에 보이는 과거 row version을 재구성한다. `COMMIT`은 해당 undo가 즉시 사라진다는 뜻이 아니다. 가장 오래된 read view가 더 이상 그 version을 요구하지 않아야 purge가 정리하며, purge로 논리적으로 재사용 가능해지는 것과 undo tablespace file이 물리적으로 작아지는 truncate는 서로 다른 단계다.

#### rollback용 기록과 MVCC용 version history

“변경 전 데이터를 별도 백업한다”는 설명은 좋은 첫 그림이지만, 실제로는 table 전체 사본을 만드는 것이 아니라 **clustered index record의 최근 변경을 되돌리거나 이전 version을 재구성하는 데 필요한 undo record**를 남긴다.

```mermaid
flowchart LR
    DML["INSERT·UPDATE·DELETE"] --> CUR["buffer pool의 current row 변경"]
    DML --> U["undo record 생성"]
    U --> RB["ROLLBACK<br/>변경 취소"]
    U --> MVCC["consistent read<br/>read view에 맞는 과거 version 재구성"]
    COMMIT["COMMIT"] --> HIST["필요한 update undo는 history list에 남음"]
    MVCC --> VIEW["oldest read view가 더는 필요로 하지 않음"]
    HIST --> VIEW
    VIEW --> PURGE["purge thread가 정리<br/>공간 재사용 가능"]
```

- `INSERT` undo는 주로 rollback에 필요하므로 transaction이 commit되면 다른 transaction의 consistent read가 참조할 이유가 없다.
- `UPDATE·DELETE` 계열 undo는 rollback뿐 아니라 이전 version을 읽는 MVCC에도 사용될 수 있어 commit 뒤에도 history에 남을 수 있다.
- undo는 redo의 반대말처럼 보이지만 책임이 다르다. **redo는 crash 뒤 committed 변경을 다시 적용**하고, **undo는 미완료 변경을 취소하거나 과거 version을 재구성**한다.

> [!warning] “100GB DELETE = 정확히 100GB undo”로 계산하지 않는다
> 대량 DELETE는 같은 규모의 매우 큰 undo와 redo·binary log·replication lag를 만들 수 있다는 경고로 기억한다. 다만 undo record format, row와 index 구성, transaction 형태가 달라지므로 원본 data byte와 undo byte가 정확히 1:1이라는 공식은 아니다. 운영에서는 큰 DELETE를 작은 batch로 나누고 transaction마다 commit해 undo peak와 lock 보유 시간을 제한한다.

#### 4.2.9.1 언두 로그 모니터링

핵심은 undo file 크기 하나가 아니라 **가장 오래된 transaction·read view, History list length의 추세, purge가 따라가는 속도, disk 여유**를 같은 시간축에서 보는 것이다.

```mermaid
sequenceDiagram
    participant A as Tx A: 오래 열린 RR read view
    participant B as Tx B
    ...
    B->>B: UPDATE 후 COMMIT
    C->>C: DELETE 후 COMMIT
    P-->>B: A가 볼 수 있는 version이라 정리 보류
    P-->>C: A가 볼 수 있는 version이라 정리 보류
    A->>A: COMMIT 또는 ROLLBACK
    P->>B: 더는 필요 없는 undo purge
    P->>C: 더는 필요 없는 undo purge
```

`COMMIT`은 “내 변경이 확정됐다”는 뜻이지 “내가 만든 undo가 즉시 삭제됐다”는 뜻이 아니다. Tx A의 오래된 read view가 Tx B·C의 변경 전 version을 볼 가능성이 있으면 purge horizon이 A 앞에 멈춘다. 심지어 변경하지 않는 **장기 read-only transaction**도 이 문제를 만들 수 있다.

```sql
-- 오래 열린 transaction과 변경량 확인
SELECT trx_id,
       trx_state,
       trx_started,
       TIMESTAMPDIFF(SECOND, trx_started, NOW()) AS age_seconds,
...
-- TRANSACTIONS 구역의 History list length·read view·purge 진행 확인
SHOW ENGINE INNODB STATUS\G

-- undo tablespace file·크기 확인
SELECT tablespace_name, file_name, extent_size, initial_size, data_free
FROM information_schema.files
WHERE file_type = 'UNDO LOG';

-- MySQL 8.0 undo tablespace 상태 변수
SHOW STATUS LIKE 'Innodb_undo_tablespaces%';
```

> [!tip] 장애 대응 순서
> 1. `INNODB_TRX`에서 가장 오래된 transaction과 application owner를 찾는다.
> 2. `History list length`가 계속 증가하는지, disk와 latency도 나빠지는지 본다.
> 3. 정상 종료 가능한 transaction이면 application에서 commit·rollback한다. 강제 종료는 rollback 비용과 업무 영향을 먼저 판단한다.
> 4. 대량 DML은 PK range batch로 나누고 batch마다 commit한다.
> 5. purge가 따라잡을 시간을 주고 재사용 여부를 확인한다. undo file을 OS에서 직접 삭제하면 안 된다.

단일 `History list length` 숫자에 보편적인 정상·비정상 임계값은 없다. **workload가 끝난 뒤 줄어드는가**, oldest transaction age와 함께 계속 커지는가, disk가 고갈되는가가 더 중요하다.

#### 4.2.9.2 언두 테이블스페이스 관리

MySQL 8.0.14부터 undo log는 항상 system tablespace 밖의 undo tablespace에 저장된다. instance 초기화 시 기본 두 개가 만들어진다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-14-언두-테이블스페이스-구조.jpg]]

```text
data dictionary name : innodb_undo_001, innodb_undo_002
기본 file name       : undo_001, undo_002
기본 위치            : innodb_undo_directory, 미지정 시 data directory
```

```mermaid
flowchart TB
    TS["undo tablespace<br/>기본 2개, 전체 최대 127개"] --> RS["rollback segments<br/>tablespace당 기본·최대 128"]
    RS --> SLOT["undo slots<br/>page size ÷ 16 bytes"]
    SLOT --> UL["transaction의 undo logs<br/>일반·temporary table 작업별 할당"]
    EX["16KiB page"] --> CALC["rollback segment당 1024 slots"]
```

책의 계산식은 **undo slot 수로부터 얻는 이론상 할당 상한**이다. 실제 동시 transaction 수를 보장하는 capacity 공식은 아니다. 한 transaction이 INSERT·UPDATE·DELETE와 temporary table 작업 조합에 따라 여러 undo log를 사용할 수 있고, connection·lock·memory·CPU·I/O 같은 다른 자원이 더 먼저 한계가 될 수 있다.

##### purge와 truncate를 구분한다

```text
purge    = 더는 필요 없는 undo record를 논리적으로 정리해 내부 공간을 재사용 가능하게 함
truncate = 커진 undo tablespace를 초기 크기 수준으로 다시 만들어 물리 file 공간을 반환함
```

자동 truncate는 `innodb_undo_log_truncate=ON`일 때 대상 tablespace를 차례로 inactive 처리하고 purge로 비운 뒤 truncate하고 다시 active로 돌린다. 이 과정에는 **최소 두 개의 active undo tablespace**가 필요하다. 수동으로 하나를 inactive 상태로 빼면서 자동 truncate 조건도 유지하려면 총 세 개 이상을 두라는 책의 설명은 “하나는 작업 중, 둘은 active 유지”라는 뜻이다.

| 시스템 변수 | 실습 서버 8.0.44 | 동적 | 기억할 점 |
|---|---:|:---:|---|
| `innodb_undo_tablespaces` | `2` | 조정 불가 | 8.0.14부터 deprecated·not configurable. 현재 값을 보고 설정 대상으로 착각하지 않는다. |
| `innodb_rollback_segments` | `128` | 예 | 각 undo tablespace와 global temporary tablespace의 rollback segment 수. 기본값이자 최댓값 128. |
| `innodb_undo_log_truncate` | `ON` | 예 | 자동 undo tablespace truncate 활성화. 최소 두 active tablespace 필요. |
| `innodb_max_undo_log_size` | `1 GiB` | 예 | 자동 truncate 대상 선정을 위한 크기 경계. undo 사용량 제한값이 아니다. |
| `innodb_undo_directory` | `./` | 아니오 | 기본 undo tablespace 위치. startup 전에 정하고 known directory 조건을 지킨다. |
| `innodb_purge_threads` | `4` | 아니오 | history의 불필요한 undo와 delete-marked record를 정리하는 background thread 수. |
| `innodb_purge_batch_size` | `300` | 예 | 한 purge batch의 undo page 단위 작업량. 고급 튜닝 영역. |
| `innodb_purge_rseg_truncate_frequency` | `128` | 예 | purge iteration 중 rollback segment를 truncate 대상으로 검사하는 빈도. |
| `innodb_max_purge_lag` | `0` | 예 | history lag가 기준을 넘을 때 DML 지연을 유도하는 고급 보호 장치. `0`은 제한하지 않음. |
| `innodb_max_purge_lag_delay` | `0` | 예 | 위 보호 장치가 DML에 줄 수 있는 최대 지연 microseconds. |

변수를 바꾸기 전에 먼저 장기 transaction과 batch 크기라는 원인을 해결한다. purge thread나 lag 변수를 무작정 키우는 것은 오래된 read view가 purge horizon을 막은 상황을 해결하지 못한다.

##### 사용자 생성 undo tablespace의 생명주기

```sql
-- 현재 undo tablespace와 file 확인
SELECT tablespace_name, file_name
FROM information_schema.files
WHERE file_type = 'UNDO LOG';

-- 8.0.14+: 추가 생성. file 확장자는 .ibu
CREATE UNDO TABLESPACE extra_undo_003
  ADD DATAFILE '/known/path/undo_003.ibu';

-- 새 transaction 할당 중지
ALTER UNDO TABLESPACE extra_undo_003 SET INACTIVE;

-- purge가 모두 비워 STATE=empty가 됐는지 확인
SELECT name, state
FROM information_schema.innodb_tablespaces
WHERE name = 'extra_undo_003';

-- 사용자 생성 tablespace만 drop 가능
DROP UNDO TABLESPACE extra_undo_003;

-- drop하지 않고 다시 사용할 수도 있음
ALTER UNDO TABLESPACE extra_undo_003 SET ACTIVE;
```

- `.ibu` file의 full path는 `innodb_directories` 등 InnoDB가 아는 경로여야 한다.
- `SET INACTIVE` 직후 곧바로 file을 지우는 것이 아니다. 기존 transaction 종료 → purge → `empty` 상태를 기다린다.
- 기본 `innodb_undo_001`, `innodb_undo_002`는 drop할 수 없다. 사용자 생성 tablespace만 empty 상태에서 drop한다.
- undo I/O pattern은 SSD에 두기 좋은 후보지만, 별도 storage 배치는 recovery 시 file 발견 경로와 장애 설계까지 함께 검증한다.
...
innodb_undo_001 → ./undo_001 → 16 MiB
innodb_undo_002 → ./undo_002 → 16 MiB
innodb_rollback_segments                       = 128
innodb_undo_log_truncate                       = ON
innodb_max_undo_log_size                       = 1 GiB
innodb_purge_threads                           = 4
관측 시점 active transaction                   = 0
관측 시점 History list length                  = 0
관측 시점 open read views                      = 0
```

이 값은 idle 시점의 snapshot일 뿐 “운영 용량이 충분하다”는 증거가 아니다. 학습 실험에서는 장기 RR transaction을 열고 다른 session에서 반복 UPDATE한 뒤 `History list length` 증가와 transaction 종료 후 감소를 관찰하면 undo·read view·purge의 연결을 확인할 수 있다.

### 4.2.10 체인지 버퍼

> [!summary] 30초 요약
> change buffer는 **buffer pool에 없는 secondary index page의 변경을 즉시 읽어 적용하지 않고 변경 조각만 먼저 기록**해 random read를 줄이는 구조다. 이후 그 page가 buffer pool로 읽히거나 background merge가 실행될 때 원래 index page에 합친다. 작업을 없애는 것이 아니라 I/O를 뒤로 미루고 묶는 최적화이며, UNIQUE index처럼 결과 반환 전에 중복 여부를 확인해야 하는 index에는 일반적으로 사용할 수 없다.

#### 무엇을 미루고 무엇은 미루지 않는가

table row가 저장된 clustered index와 secondary index는 DML 한 번에 함께 바뀔 수 있다. index가 많을수록 변경해야 할 B+Tree가 늘어나므로 write amplification이 커진다. 그중 buffer pool에 없는 secondary leaf page를 매번 disk에서 random read하면 foreground DML latency가 증가한다. change buffer는 이 **page read와 secondary page update를 지연**한다.

```mermaid
flowchart TD
    DML["INSERT·UPDATE·DELETE"] --> CL["clustered index row 변경"]
    DML --> SEC["영향받는 secondary index마다 변경"]
    SEC --> HIT{"target page가<br/>buffer pool에 있는가?"}
    HIT -->|"예"| NOW["page 즉시 변경 → dirty page"]
    HIT -->|"아니오"| ELIG{"change buffering<br/>가능한 index인가?"}
    ELIG -->|"아니오: UNIQUE·descending 예외 등"| READ["disk에서 page를 읽어<br/>검사·즉시 변경"]
    ELIG -->|"예"| CB["change buffer에<br/>index 변경 조각 기록"]
    CB --> RETURN["foreground DML 완료 경로"]
    CB --> MERGE["page read·background 작업·slow shutdown에서 merge"]
    MERGE --> PAGE["원래 secondary index page 변경"]
    PAGE --> FLUSH["dirty page가 나중에 disk로 flush"]
```

change buffer는 data row 전체나 clustered index 변경을 대신 보관하는 공간이 아니다. 또한 “임시 메모리 공간”으로만 외우면 부족하다.

- memory에서는 buffer pool의 일부를 사용한다.
- disk에서는 system tablespace에 change buffer 내용을 두므로 commit이나 server restart 뒤에도 미완료 merge가 이어질 수 있다.
- foreground I/O를 없애는 것이 아니라 **merge debt로 전환**한다. 이후 page read와 background merge에 CPU·I/O가 필요하다.
- `innodb_change_buffering=none`으로 바꿔도 이미 쌓인 entry가 즉시 사라지는 것은 아니다. 새 buffering을 막고 기존 entry는 계속 merge된다.

#### UNIQUE index는 왜 일반적으로 제외되는가

원문의 “유닉스 인덱스”는 **UNIQUE index**를 뜻한다. UNIQUE secondary index는 새 key를 성공으로 반환하기 전에 같은 key가 이미 존재하는지 확인해야 한다. target page를 읽지 않은 채 “나중에 merge하자”고 응답하면 merge 시점에야 중복을 발견할 수 있어 UNIQUE constraint의 즉시 보장을 깨뜨린다. 그래서 일반적인 `unique_checks=ON`에서는 change buffering 대상이 아니다.

> [!warning] `unique_checks=0`은 보편적인 해결책이 아니다
> bulk load에서 data가 정말 unique하다는 외부 보장이 있을 때 검사 비용을 줄이기 위해 사용할 수 있고, 이때 change buffering이 더 활용될 수 있다. 하지만 중복 data를 받아들일 위험이 있으므로 application의 일반 DML이나 출처가 불명확한 data에는 사용하지 않는다.

공식 문서상 secondary index에 descending column이 있거나 PK가 descending column을 포함하는 경우도 change buffering 대상에서 제외된다.

#### merge는 언제 일어나는가

책의 “change buffer merge thread”는 **background에서 merge되는 동작**을 기억하기 위한 표현으로는 유용하다. 다만 반드시 하나의 전용 thread만이 모든 merge를 수행한다고 고정해 외우지는 않는다.

- 해당 secondary index page가 query 때문에 buffer pool로 읽힐 때 change entry를 적용한다.
- server가 한가할 때 InnoDB main thread 등 background 작업이 merge를 진행한다.
- purge 과정과 slow shutdown도 merge를 진행할 수 있다.
- transaction `COMMIT`은 관련 change entry가 원래 page에 모두 merge됐다는 뜻이 아니다.

따라서 대량 write 직후에는 foreground latency가 좋아져도 이후 merge I/O가 길게 이어질 수 있다. shutdown을 서두르거나 storage가 이미 포화된 상황에서는 merge 비용도 함께 본다.

#### `innodb_change_buffering` 옵션

첨부 사진의 옵션은 다음처럼 읽는다. `delete`는 한 번에 사라지는 것이 아니라 **delete-mark → purge의 physical delete**라는 두 단계임에 주의한다.

| 값 | buffering하는 작업 |
|---|---|
| `all` | insert + delete-mark + purge의 physical delete |
| `none` | 새 change buffering을 하지 않음 |
| `inserts` | secondary index item 삽입 |
| `deletes` | secondary index item에 delete mark 표시 |
| `changes` | insert + delete-mark |
| `purges` | purge가 delete-marked item을 물리 삭제하는 작업 |

| 시스템 변수 | 실습 서버 8.0.44 | 범위·동적 | 기억할 점 |
|---|---:|---|---|
| `innodb_change_buffering` | `all` | GLOBAL·동적 | 새로 buffer할 operation 종류. 변경해도 기존 entry의 merge는 계속된다. |
| `innodb_change_buffer_max_size` | `25` | GLOBAL·동적 | buffer pool에서 change buffer가 사용할 수 있는 최대 비율(%). 기본 25, 최대 50. pre-allocation이나 고정 점유량이 아니다. |
| `unique_checks` | `ON` | GLOBAL·SESSION·동적 | UNIQUE 검사 여부. bulk load의 검증된 data가 아니라면 OFF로 두지 않는다. |

`innodb_change_buffer_max_size`를 키우면 공짜 memory가 생기는 것이 아니다. 같은 buffer pool에서 data·index cache가 사용할 공간을 줄일 수 있으므로 read hit rate와 change buffer benefit을 함께 측정한다.

#### 효과가 큰 조건과 오히려 불리한 조건

| 효과가 커질 가능성 | 이득이 작거나 불리할 가능성 |
|---|---|
| write-heavy·I/O-bound | working set이 buffer pool에 대부분 상주 |
| non-unique secondary index가 많음 | secondary index가 적거나 대부분 UNIQUE |
| target secondary leaf page가 자주 cache miss | read-heavy라 data page cache가 더 중요 |
| bulk insert처럼 변경을 묶어 merge할 여지가 큼 | merge debt 때문에 후속 I/O·shutdown 시간이 문제 |

HDD에서는 random I/O 지연 효과가 직관적으로 크지만 SSD라고 무조건 끌 이유도, 켤 이유도 없다. storage latency·buffer pool 압력·DML p95/p99·merge 추세로 판단한다. 특히 index가 많다는 근본 비용을 change buffer가 제거하지 않으므로 사용하지 않는 index부터 줄이는 것이 우선이다.
...
-- INSERT BUFFER AND ADAPTIVE HASH INDEX 구역의 Ibuf size·merge 내역 확인
SHOW ENGINE INNODB STATUS\G
```

현재 `realmysql 8.0.44` Community Server의 읽기 전용 snapshot은 다음과 같다.
...
이 수치는 지금까지의 workload에서 merge가 관측되지 않았다는 뜻일 뿐 기능이 불필요하다는 증거는 아니다. 실험하려면 buffer pool보다 큰 table에 non-unique secondary index를 만들고 충분한 bulk DML을 실행한 뒤, `ibuf_*` 변화·disk read/write·buffer pool cache·DML latency·merge가 따라잡는 시간을 함께 비교한다.

### 4.2.11 리두 로그 및 로그 버퍼

> [!summary] 30초 요약
> redo log는 **commit된 변경을 crash 후 다시 실행할 수 있게 하는 disk log**로 ACID의 Durability를 받쳐준다. InnoDB는 data page를 매 commit마다 쓰지 않고, 변경 정보를 log buffer에 만든 뒤 WAL 규칙에 따라 redo를 data page보다 먼저 durable하게 한다. `innodb_flush_log_at_trx_commit`은 commit latency와 crash 시 data-loss window를 바꾸며, 기본 `1`이 full ACID 선택이다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-15-리두-WAL과-crash-recovery.JPG]]

#### "log를 먼저 기록"의 정확한 뜻: WAL

WAL(Write-Ahead Logging)은 변경된 data page를 disk에 쓰기 전에, **그 page 변경을 재현할 redo record가 먼저 durable해야 한다**는 순서 규칙이다. SQL 문장 자체나 row 전체를 그대로 복사한다는 뜻은 아니다.
...
sequenceDiagram
    ...
    participant R as "redo log files"
    participant D as "data files"

    A->>I: "DML + COMMIT"
    I->>B: "data/index page 변경 (dirty page)"
    I->>L: "redo record 생성"
    L->>R: "write + flush (설정에 따름)"
    R-->>I: "durable LSN"
    I-->>A: "COMMIT 성공"
    Note over B,D: "data page는 commit 후에도 memory에 남을 수 있음"
    B->>D: "page cleaner·checkpoint가 나중에 batch flush"
```

이 분리 덕분에 commit마다 16KiB data page 여러 개를 random write할 필요가 없다. 작고 순차적인 redo write로 commit을 보호하고, dirty page는 background에서 묶어 기록한다. 단, redo와 data file이 같은 storage에서 경합하거나 flush latency가 튀면 commit latency도 영향을 받는다.

#### crash recovery에서 redo와 undo가 함께 필요한 이유

InnoDB의 fuzzy checkpoint는 server를 정지하지 않고 dirty page를 계속 flush한다. 그래서 crash 시 data file에는 각 page별로 서로 다른 시점의 내용이 있을 수 있다.

```mermaid
flowchart TD
    C["crash"] --> CP["가장 최근 checkpoint LSN 확인"]
    CP --> RR["redo roll-forward<br/>durable redo를 page에 재적용"]
    RR --> TX{"crash 시점에<br/>transaction 상태"}
    TX -->|"COMMIT 완료"| KEEP["변경 유지"]
    TX -->|"COMMIT 미완료"| UR["undo rollback<br/>이전 row version으로 복구"]
    KEEP --> OK["일관된 상태"]
    UR --> OK
```

- **commit됐지만 data page에 아직 없는 변경**은 redo로 roll-forward한다.
- **commit되지 않았는데 data page에 일부 쓰인 변경**은 redo로 복구 기준을 맞춘 뒤 undo로 rollback한다.
- redo는 committed transaction만의 로그가 아니다. 미완료 transaction의 redo도 있을 수 있으므로, recovery는 redo 재실행과 undo 취소를 함께 사용한다.

| 구조 | 질문 | 주요 용도 | 장애 후 역할 |
|---|---|---|---|
| redo log | 변경을 다시 적용할 수 있는가? | WAL·Durability·checkpoint | durable한 변경 roll-forward |
| undo log | 변경을 취소하거나 과거 version을 보여줄 수 있는가? | rollback·MVCC | 미완료 transaction rollback |
| binary log | MySQL server 차원의 변경 event를 전파·복원할 수 있는가? | replication·PITR | backup 후 특정 시점 복원 |

redo는 backup이 아니다. tablespace 손실·사용자의 잘못된 `DELETE`·원하는 시점 복원은 backup과 binary log 정책이 별도로 필요하다.

#### log buffer와 commit flush 정책

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-16-리두-flush-정책과-log-buffer.jpg]]

log buffer는 disk redo file에 쓸 redo record를 잠시 담는 **global memory area**다. `innodb_log_buffer_size`가 작은데 하나의 큰 transaction이 redo를 많이 만들면 COMMIT 전에도 buffer 공간을 비우기 위해 redo write가 필요할 수 있다. 큰 batch DML·BLOB·TEXT 변경이 있을 때 크기 확대를 검토하지만, 일반 OLTP에서 무조건 키우는 영역은 아니다.

`write`는 application memory의 log buffer를 OS page cache·storage stack으로 넘기는 단계이고, `flush`/`fsync`는 전원 상실 후에도 남도록 storage에 durable함을 요청하는 단계다. 두 단계를 구분해야 `0·1·2`를 이해할 수 있다.

| `innodb_flush_log_at_trx_commit` | COMMIT 시 | 주기 작업 | crash 위험 | 사용 판단 |
|---:|---|---|---|---|
| `1` | redo write + disk flush | commit마다 | storage가 flush를 정직하게 보장하면 committed transaction 보호 | 기본·full ACID·운영 권장 |
| `0` | commit 자체는 write·flush를 강제하지 않음 | 약 N초마다 write + flush | mysqld·OS·전원 crash에서 최근 N초 유실 가능 | 명시적 data-loss 허용 시만 |
| `2` | commit마다 write, disk flush는 안 함 | 약 N초마다 flush | mysqld만 crash하면 OS cache에 남을 수 있지만 OS·전원 crash에서 최근 N초 유실 가능 | `0`보다 작은 타협이지만 full ACID는 아님 |

`N`은 `innodb_flush_log_at_timeout`이고 기본 1초다. 실제 flush는 DDL·InnoDB 내부 작업으로 더 자주, scheduler 지연으로 더 느리게 발생할 수 있어 정확한 1초 timer로 보장되지 않는다. replication·PITR 일관성이 중요하면 `innodb_flush_log_at_trx_commit=1`과 `sync_binlog=1`을 함께 본다.

> [!warning] `fsync 성공 = 절대적인 물리 영속성`은 아니다
> OS·disk controller·storage cache가 flush 완료를 정직하게 보장해야 한다. 데이터 무결성이 중요한 환경은 storage 캐시 정책·전원 보호·backup·replication까지 같이 검증한다.

#### redo capacity와 buffer pool은 같은 축이 아니다

더 큰 buffer pool은 더 많은 page를 cache할 수 있다. 더 큰 redo capacity는 변경 burst에서 dirty page를 즉시 flush하지 않고 더 오래 두었다가 묶어 쓸 수 있는 여유를 만든다. capacity가 너무 작으면 checkpoint 압력과 flush burst가 생기고, 너무 크면 crash recovery에서 읽어야 할 redo 범위가 커질 수 있다.

> [!warning] 책과 현재 MySQL 8.0의 버전 차이
> 책의 `innodb_log_file_size × innodb_log_files_in_group`은 8.0.30 이전의 redo capacity 모델이다. 8.0.30+는 dynamic GLOBAL 변수 `innodb_redo_log_capacity`가 대체하며, `#innodb_redo` 디렉터리의 ordinary·spare file 약 32개로 관리한다. 기존 두 변수는 deprecated되었다.

#### 4.2.11.1 리두 로그 아카이빙: 왜 필요한가?

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-17-ACID와-redo-archiving.jpg]]

redo file은 순환 재사용되므로 checkpoint가 지난 공간은 새 log에 덮어써진다. 물리 backup 도구가 data file을 복사하는 동안 변경량이 많아 도구가 필요한 redo를 따라잡기 전에 덮어쓰면 backup의 일관성을 만들 수 없다. redo archiving은 새 redo record를 별도 file에 순차적으로 복사해 이 간격을 막는다.

- 목적은 **backup 진행 중 필요한 redo를 순환 덮어쓰기에서 보존**하는 것이다.
- binary log 보관·PITR을 대체하는 일반 history archive가 아니다.
- `innodb_redo_log_archive_dirs` 설정 후 `innodb_redo_log_archive_start()`를 호출한 session을 계속 유지해야 하며, 같은 session이 `..._stop()`을 호출한다.
- archive storage가 느리면 transactional logging을 throttle할 수 있고, archive file은 사용 후 운영자가 정리해야 한다.

#### 4.2.11.2 redo 활성화·비활성화

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-18-redo-활성화와-비활성화.jpg]]

MySQL 8.0.21+는 **새 instance에 대량 data를 적재하는 특수 용도**로 redo logging을 끄고 켤 수 있다.

```sql
ALTER INSTANCE DISABLE INNODB REDO_LOG;
-- 새 instance에 bulk load
ALTER INSTANCE ENABLE INNODB REDO_LOG;

SHOW GLOBAL STATUS LIKE 'Innodb_redo_log_enabled';
```

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-19-redo-비활성화-위험.jpg]]

> [!danger] 운영 server에서 redo를 끄지 않는다
> disabled 상태에서 비정상 종료하면 data 유실·instance 손상이 생기고 정상 시작을 거부할 수 있다. 공식 문서는 운영 server에서 사용하지 말라고 명시한다. 이 명령은 `INNODB_REDO_LOG_ENABLE` 권한과 exclusive backup lock을 필요로 한다. 단순히 최근 N초 data 유실을 감수하는 정도라면 redo 전체를 끄는 대신 flush 정책 `0`·`2`를 명시적으로 검토한다.

#### 현재 실습 server 8.0.44 관측

`realmysql` container에서 읽기 전용으로 확인한 snapshot이다.

| 항목 | 현재 값 | 해석 |
|---|---:|---|
| `innodb_flush_log_at_trx_commit` | `1` | commit마다 redo write + flush |
| `innodb_flush_log_at_timeout` | `1` | `0`·2의 periodic flush 기본 간격 |
| `innodb_log_buffer_size` | `16 MiB` | memory log buffer |
| `innodb_redo_log_capacity` | `100 MiB` | 8.0.30+ redo capacity 설정 |
| `Innodb_redo_log_capacity_resized` | `100 MiB` | 실제 적용된 capacity |
| `Innodb_redo_log_enabled` | `ON` | redo logging 활성 |
| `sync_binlog` | `1` | binary log도 commit durability에 맞춘 상태 |
| `innodb_redo_log_archive_dirs` | 빈 값 | redo archive 미구성 |
| deprecated 두 변수 | `48 MiB × 2` | 표시는 되지만 capacity는 `innodb_redo_log_capacity` 100 MiB가 지배 |

값이 기본이라고 현재 workload에 최적이라는 뜻은 아니다. commit latency·redo bytes/s·checkpoint age·dirty page flush·crash recovery 목표를 같은 시간축으로 보고 조정한다.

### 4.2.12 어댑티브 해시 인덱스(Adaptive Hash Index)

> [!summary] 30초 요약
> AHI는 사용자가 DDL로 만드는 index가 아니라, InnoDB가 **buffer pool에 상주한 index page의 반복 검색 pattern을 관찰해 자동으로 만드는 memory hash shortcut**이다. 잘 맞으면 B+Tree의 root→branch→leaf 탐색과 관련 CPU·latch 작업을 줄인다. 그러나 disk I/O를 대신하지 않고 range search에도 맞지 않으며, hash 유지 비용·memory·partition latch contention이 생길 수 있다. 따라서 ON/OFF를 정답처럼 외우지 않고 실제 workload를 같은 조건에서 benchmark한다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-20-AHI-생성과-BTree-우회.jpg]]

#### schema index와 무엇이 다른가

일반 InnoDB index는 사용자가 `PRIMARY KEY`, `UNIQUE`, `INDEX`로 정의하며 disk의 B+Tree 구조로 유지된다. AHI는 그 B+Tree를 없애거나 대체하는 별도 schema index가 아니다.

- InnoDB가 반복 검색을 관찰해 자주 접근하는 index page에 대해 필요할 때 만든다.
- 전체 index key일 수도 있고 일부 key prefix일 수도 있으며, 한 B+Tree의 모든 값이 반드시 들어가는 것도 아니다.
- 개념적으로 `(B+Tree index ID, 관찰된 key prefix) → buffer pool의 record/page 위치` shortcut을 둔다.
- server restart나 AHI 비활성화로 사라져도 원본 B+Tree가 있으므로 data 정확성에는 영향이 없다. 다시 활성화하면 workload를 관찰하며 재구축한다.
- optimizer가 실행 계획에서 선택하는 별도 access path가 아니라 InnoDB 내부 탐색 최적화이므로 `EXPLAIN`에 `AHI 사용`이라고 표시되지 않는다.

```mermaid
flowchart TD
    Q["동일·유사 key lookup 반복"] --> BT["B+Tree root → branch → leaf 탐색"]
    BT --> OBS["InnoDB가 검색 pattern과 hot index page 관찰"]
    OBS --> BUILD{"AHI 생성이 이득이라고 판단?"}
    BUILD -->|"아니오"| BT
    BUILD -->|"예"| HASH["(index ID + key prefix) → hot page/record pointer"]
    NEXT["다음 equality-like lookup"] --> HIT{"AHI hit?"}
    HIT -->|"예"| DIRECT["root·branch traversal 우회"]
    DIRECT --> PAGE["buffer pool의 target page/record"]
    HIT -->|"아니오"| BT
```

AHI hit가 빠른 이유는 algorithm 이름만 `hash`여서가 아니라, 이미 memory에 있는 page를 대상으로 B+Tree 여러 level을 내려가며 비교하고 latch를 잡는 일을 줄이기 때문이다. 따라서 **B+Tree 탐색 CPU를 줄이는 기능**이지 **disk에서 page를 읽지 않게 해 주는 cache 자체**는 아니다.

#### 잘 맞는 query와 구조적으로 맞지 않는 query

| 이득 가능성이 큰 조건 | 이득이 작거나 손해가 될 수 있는 조건 |
|---|---|
| working set이 buffer pool에 대부분 상주 | buffer pool miss와 disk read가 지배적 |
| 동일 key·key prefix의 반복 equality lookup | range scan·넓은 scan·leading wildcard `LIKE '%x'` |
| `=`·좁은 `IN`처럼 hot key가 반복됨 | 접근 key가 계속 달라져 재사용성이 낮음 |
| CPU가 B+Tree 탐색에 많이 쓰임 | write·DDL이 많아 AHI 유지·제거 비용이 큼 |
| 특정 data page에 읽기가 집중됨 | 다수 concurrent join 등으로 AHI latch contention이 큼 |

`IN` 자체가 AHI 전용 operation인 것은 아니다. 내부적으로 여러 equality lookup으로 분해되고 각 lookup이 반복되는 hot key pattern일 때 도움을 받을 가능성이 있다는 뜻이다. 반대로 range와 정렬에는 hash의 순서 정보가 없으므로 원본 B+Tree가 필요하다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-21-AHI-성능-비교.jpg]]

책의 graph는 특정 환경에서 AHI 활성화 뒤 CPU 사용률이 낮아지고 처리량이 늘어난 예시다. 이를 보편적인 배수로 외우지 않는다. table size·buffer pool 적중률·key 분포·동시성·query mix가 달라지면 반대 결과도 나올 수 있다.

#### 공짜가 아닌 이유: memory·유지 작업·latch contention

AHI도 lookup pattern을 관찰하고 hash entry를 만들고 지우며, data page·index 변화에 맞춰 관리해야 한다.

- AHI key와 pointer를 위한 memory가 필요하다.
- miss라면 AHI 확인 뒤 원본 B+Tree를 다시 탐색하므로 유효 hit가 적을 때 overhead가 남는다.
- 각 AHI partition은 별도 latch로 보호된다. 많은 thread가 같은 partition에 몰리면 CPU가 높아지고 wait가 생길 수 있다.
- table `DROP`·`ALTER` 등은 관련 AHI entry를 정리해야 하므로 entry가 많을수록 DDL 종료 시간이 늘 수 있다.

`innodb_adaptive_hash_index_parts`는 이런 경합을 나누는 partition 수다. MySQL 8.0 기본은 8, 범위는 1~512이며 **startup에서 정하는 static 변수**다. index마다 한 partition에 연결되므로 partition을 늘리면 경합을 완화할 수 있지만 memory·관리 overhead가 늘고, 불필요한 AHI 자체를 끄는 것보다 항상 낫다고 할 수 없다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-22-AHI-유불리-조건.jpg]]

> [!warning] version별 기본값을 구분한다
> 책과 MySQL 8.0에서는 `innodb_adaptive_hash_index=ON`이 기본이다. MySQL 8.4에서는 기본값이 `OFF`로 바뀌었다. 따라서 “MySQL은 기본 ON”을 모든 version에 적용하지 말고, 실제 instance에서 변수를 확인한다.

#### Spring·JPA에서는 어떻게 보이는가

> [!answer] 여백 질문 답
> Spring Data JPA나 Hibernate가 AHI를 직접 만들거나 query별로 선택하지 않는다. application은 평소처럼 SQL을 보내고 optimizer는 schema의 B+Tree index를 기준으로 plan을 세운다. 그 뒤 InnoDB가 page를 찾는 내부 과정에서 AHI를 투명하게 사용할 수 있다. 따라서 JPA annotation·repository method로 AHI를 켜는 설정은 없으며, application은 생성 SQL의 equality/range 형태와 key 편중을 만들고, DBA·server 설정과 실제 metric이 AHI 사용 여부를 결정한다.

즉 Spring backend가 할 일은 `EXPLAIN`으로 올바른 schema index를 먼저 확인하고, AHI는 DB 전체 workload 관점에서 별도로 측정하는 것이다. 특정 repository query 하나가 빨라졌다는 이유만으로 global AHI를 켜거나 끄면 다른 service query에 반대 영향이 생길 수 있다.

#### 관측: 사용량과 contention을 분리한다

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-23-AHI-모니터링.jpg]]
...
| 관측값 | 알려 주는 것 | 이것만으로 결론 내릴 수 없는 것 |
|---|---|---|
| `hash searches/s` | 최근 monitor 구간에서 AHI로 처리된 search rate | query latency가 좋아졌는지 |
| `non-hash searches/s` | 원본 B+Tree 등 non-AHI search rate | AHI를 끄면 더 빨라질지 |
| `SEMAPHORES`의 `btr0sea.c` rw-latch wait | AHI 관련 contention 징후 | wait가 application 전체 병목인지 |
| `memory/innodb/adaptive hash index` | Performance Schema가 계측한 AHI memory | 모든 내부 page·구조를 합친 완전한 RSS |
| CPU·QPS·p95/p99 | 사용자에게 보이는 처리 비용·성과 | 변화 원인이 AHI 하나뿐인지 |

`hash / (hash + non-hash)`는 사용 비중의 한 단서일 뿐 효율 공식이 아니다. hit 비율이 높아도 latch contention이 더 크면 손해일 수 있고, 낮더라도 중요한 hot query의 CPU를 크게 줄일 수 있다. 반드시 같은 대표 workload에서 ON/OFF를 비교한다.

공식 문서는 AHI 관련 rw-latch 대기가 `btr0sea.c`에서 많이 보이면 partition 증가 또는 AHI 비활성화를 검토하라고 안내한다.

![[assets/book-photos/real-mysql-8-0-1/04-아키텍쳐/4.2-InnoDB-스토리지-엔진-아키텍쳐/04-24-AHI-메모리-확인.jpg]]

#### 활성화·비활성화와 실습 server snapshot
...
`innodb_adaptive_hash_index`는 GLOBAL·dynamic이다. OFF로 바꾸면 hash table을 비우는 동안에도 일반 query는 계속되며 원본 B+Tree를 사용한다. 다시 ON으로 바꾸면 workload를 관찰하며 hash table을 재구성한다. 반면 `innodb_adaptive_hash_index_parts`는 GLOBAL이지만 dynamic이 아니므로 restart 없이 바꿀 수 없다.

현재 `realmysql` Community Server를 읽기 전용으로 확인한 값이다.

| 항목 | MySQL 8.0.44 snapshot | 해석 |
|---|---:|---|
| `innodb_adaptive_hash_index` | `ON` | AHI 활성 |
| `innodb_adaptive_hash_index_parts` | `8` | 기본 partition 수 |
| buffer pool data / total pages | `7106 / 8192` | page 상주 현황의 한 시점 |
| `memory/innodb/adaptive hash index` | `1688 bytes` | Performance Schema 계측값 |
| AHI node heap buffers | partition별 `2,1,1,1,1,1,1,46` | InnoDB monitor snapshot |
| `hash searches/s` | `0.10` | 최근 20초 평균 |
| `non-hash searches/s` | `1.60` | 최근 20초 평균 |

현재는 idle에 가까운 짧은 관측이라 AHI가 유효한지 판단할 근거가 아니다. `0.10 ÷ (0.10 + 1.60) ≈ 5.9%`라는 비율도 test workload·latency·CPU·wait가 없으면 의미가 제한된다.

#### 운영 판단 절차

```mermaid
flowchart TD
    S["대표 workload와 baseline 수집"] --> M["QPS·p95/p99·CPU·buffer pool miss<br/>hash/non-hash·btr0sea latch wait"]
    M --> A["동일 data·동일 동시성으로 AHI ON/OFF A/B"]
    A --> B{"latency·CPU 개선이<br/>contention·memory 비용보다 큰가?"}
    B -->|"예"| ON["ON 유지<br/>partition·hotspot 추세 관찰"]
    B -->|"아니오"| OFF["OFF 검토<br/>B+Tree로 fallback"]
    ON --> R["version·query mix·data size 변화 시 재측정"]
    OFF --> R
```

local 개발 DB의 단발성 `SELECT`로는 결론 내리지 않는다. production과 유사한 data distribution·connection concurrency·read/write mix로 warmup 후 측정하며, ON/OFF 전환 직후에는 hash table 구축·비우기 구간을 steady state와 분리한다.
...
undo = rollback용 변경 취소 정보 + MVCC용 이전 version 재구성 정보
INSERT undo = commit 후 consistent read가 참조하지 않음
UPDATE·DELETE undo = commit 뒤에도 old read view 때문에 history에 남을 수 있음
COMMIT ≠ undo 즉시 삭제
purge horizon = 가장 오래된 read view가 정하는 정리 경계
History list length = commit됐지만 아직 purge되지 않은 update undo history의 길이
purge = 논리적 정리·공간 재사용, truncate = 물리 file 축소
대량 DELETE = 정확한 1:1 용량 공식이 아니라 큰 undo·redo·binlog 경고
undo tablespace → rollback segment → undo slot → undo log
16KiB page = rollback segment당 1024 undo slots
undo slot 계산 = 이론상 할당 상한, 실제 동시성 capacity 보장 아님
MySQL 8.0.14+ = 기본 undo_001·undo_002 + runtime CREATE UNDO TABLESPACE
innodb_undo_tablespaces = 8.0.14부터 deprecated·조정 불가
자동 truncate = undo_log_truncate + max_undo_log_size + active tablespace 최소 2개
수동 drop = user-created만 INACTIVE → purge → EMPTY → DROP
장기 read-only transaction도 undo purge를 막을 수 있음
change buffer = buffer pool miss인 eligible secondary index 변경의 random read를 지연
change buffer ≠ 작업 제거 = foreground I/O를 merge debt로 이동
UNIQUE index = 결과 반환 전 중복 확인 필요 → 일반적으로 change buffering 불가
change buffer memory = buffer pool 일부, persistence = system tablespace
merge = page read·background·purge·slow shutdown에서 수행 가능
COMMIT ≠ change buffer merge 완료
change buffering all = inserts + delete-mark + physical purge delete
change_buffer_max_size = buffer pool 비율 상한, 고정 점유량 아님
none 전환 = 새 buffering 중지, 기존 entry는 계속 merge
index가 많을수록 write amplification 증가, change buffer는 근본 비용을 제거하지 않음
redo = crash 후 변경 재실행, undo = 미완료 변경 취소·과거 version
WAL = affected data page보다 redo record를 먼저 durable하게
DML = buffer pool dirty page + log buffer redo record
COMMIT ≠ data page flush, 기본 1에서 COMMIT = redo write + fsync 후 ACK
crash recovery = checkpoint에서 redo roll-forward + loser transaction undo rollback
log buffer = disk redo file에 쓸 redo record의 global memory buffer
flush=1 = full ACID, 0 = N초마다 write+flush, 2 = commit write + N초마다 flush
0·2의 N = innodb_flush_log_at_timeout, 정확한 timer 보장은 아님
replication durability = innodb_flush_log_at_trx_commit 1 + sync_binlog 1
redo capacity = dirty write burst 흡수 여유, buffer pool cache capacity와 별도 축
MySQL 8.0.30+ = innodb_redo_log_capacity, 이전 file_size × files_in_group는 deprecated
redo archive = physical backup이 따라잡을 때까지 덮어쓰기 전 redo를 별도 보존
redo disable = 새 instance bulk load 전용, 운영 금지·crash 시 data loss·corruption
redo ≠ backup·binlog = crash recovery 보조장치, PITR·replication·storage loss 대체 불가
AHI = InnoDB가 hot buffer-pool index page에 자동 생성하는 memory hash shortcut
AHI ≠ schema index = DDL·optimizer access path가 아니라 storage-engine 내부 최적화
AHI key ≈ B+Tree index ID + 관측된 key prefix → buffer-pool page·record pointer
AHI hit = root·branch traversal 감소, disk I/O 제거 아님
AHI 유리 = memory-resident + 반복 equality lookup + hot key
AHI 불리 = disk-bound·range/wildcard·wide scan·concurrent latch contention
AHI 비용 = 관찰·생성·유지 memory + partition latch + DDL cleanup
hash searches/s 비율 ≠ 성능 결론, latency·CPU·btr0sea wait와 ON/OFF A/B
innodb_adaptive_hash_index = GLOBAL dynamic, OFF 시 AHI를 비우고 B+Tree fallback
innodb_adaptive_hash_index_parts = GLOBAL static, MySQL 8.0 기본 8
MySQL 8.0 기본 ON, MySQL 8.4 기본 OFF
Spring/JPA = AHI 직접 제어 없음, 생성 SQL·workload shape만 간접 영향
```
...
> [!question]- undo log의 두 핵심 역할은 무엇인가?
> **답:** transaction rollback 때 DML 변경을 취소하고, MVCC consistent read 때 read view에 맞는 이전 row version을 재구성한다.

> [!question]- transaction이 COMMIT되면 그 transaction의 undo가 즉시 삭제되는가?
> **답:** 아니다. UPDATE·DELETE undo는 오래된 read view가 과거 version을 요구할 수 있어 history에 남는다. 가장 오래된 read view가 사라진 뒤에야 purge가 정리할 수 있다.

> [!question]- 변경하지 않는 장기 read-only transaction도 undo 증가의 원인이 될 수 있는가?
> **답:** 그렇다. 오래된 RR read view가 purge horizon을 붙잡으면 이후 다른 transaction이 commit한 update undo도 정리하지 못해 History list length와 undo 사용량이 증가할 수 있다.

> [!question]- “100GB를 DELETE하면 undo도 정확히 100GB”인가?
> **답:** 정확한 1:1 공식은 아니다. 대량 DELETE가 같은 규모의 큰 undo·redo·binary log를 만들 수 있다는 경고다. record format과 index·transaction 형태가 다르므로 실제 크기는 측정하고, PK range batch와 중간 commit으로 peak를 제한한다.

> [!question]- undo 증가를 볼 때 file size 외에 무엇을 함께 확인해야 하는가?
> **답:** `INNODB_TRX`의 oldest transaction age와 변경량, `SHOW ENGINE INNODB STATUS`의 History list length·open read views·purge 진행, disk 여유와 query latency를 같은 시간축에서 본다. 단일 History list length 숫자보다 증가·감소 추세가 중요하다.

> [!question]- purge와 undo tablespace truncate는 같은 작업인가?
> **답:** 아니다. purge는 더는 필요 없는 undo를 논리적으로 정리해 내부 공간을 재사용 가능하게 하고, truncate는 비워진 tablespace file을 초기 크기 수준으로 다시 만들어 물리 disk 공간을 반환한다.

> [!question]- undo tablespace의 내부 계층은 어떻게 이어지는가?
> **답:** undo tablespace 안에 rollback segment가 있고, rollback segment 안에 undo slot이 있으며, transaction의 undo log가 slot을 사용한다. 기본 16KiB page에서는 rollback segment당 `16KiB ÷ 16B = 1024` slots다.

> [!question]- undo slot 계산식으로 실제 최대 동시 transaction 수를 확정할 수 있는가?
> **답:** 아니다. slot allocation의 이론상 상한이다. 한 transaction이 여러 undo log를 쓸 수 있고 connection·lock·memory·CPU·I/O 같은 자원이 먼저 병목이 될 수 있으므로 `max_connections`나 처리량 보장값으로 사용하지 않는다.

> [!question]- MySQL 8.0.14부터 undo tablespace 관리가 어떻게 달라졌는가?
> **답:** undo가 항상 system tablespace 밖의 별도 file에 기록되고, 기본 `undo_001·undo_002` 외에 `CREATE UNDO TABLESPACE`로 runtime 추가할 수 있다. 과거의 `innodb_undo_tablespaces` 변수는 deprecated되고 더는 조정할 수 없다.

> [!question]- 자동 undo truncate에 active tablespace가 최소 두 개 필요한 이유는 무엇인가?
> **답:** 하나를 inactive로 전환해 비우고 truncate하는 동안 다른 하나가 새 transaction에 undo를 계속 제공해야 하기 때문이다. 수동으로 하나를 별도 작업하면서 active 두 개를 남기려면 총 세 개 이상을 둔다.

> [!question]- 사용자 생성 undo tablespace를 제거하는 안전한 순서는 무엇인가?
> **답:** `SET INACTIVE`로 새 transaction 할당을 막고, 기존 transaction 종료와 purge를 기다려 `STATE=empty`를 확인한 뒤 `DROP UNDO TABLESPACE`한다. 기본 `innodb_undo_001·002`는 drop할 수 없으며 OS에서 file을 직접 삭제하면 안 된다.

> [!question]- `innodb_max_undo_log_size=1GiB`는 undo가 1GiB를 넘지 못한다는 뜻인가?
> **답:** 아니다. 자동 truncate 대상으로 고르는 크기 경계다. long transaction이 purge를 막으면 undo는 그보다 더 커질 수 있으므로 용량 제한 장치로 오해하면 안 된다.

> [!question]- change buffer는 어떤 작업을 최적화하는가?
> **답:** buffer pool에 없는 eligible secondary index page를 foreground DML이 즉시 random read해 수정하는 작업을 미룬다. 변경 조각을 먼저 기록하고 page가 나중에 읽히거나 background merge가 실행될 때 원래 page에 합친다.

> [!question]- UNIQUE secondary index를 일반적으로 change buffering할 수 없는 이유는 무엇인가?
> **답:** 성공을 반환하기 전에 같은 key가 이미 있는지 확인해야 하기 때문이다. target page를 읽지 않고 변경을 미뤘다가 merge 시점에 중복을 발견하면 UNIQUE constraint를 즉시 보장할 수 없다.

> [!question]- change buffer는 휘발성 memory에만 존재하는가?
> **답:** 아니다. memory에서는 buffer pool 일부를 사용하지만 disk에서는 system tablespace에 영속화되어 server restart 뒤에도 기존 entry의 merge가 이어질 수 있다.

> [!question]- transaction이 COMMIT되면 change buffer의 변경도 원래 index page에 모두 merge된 것인가?
> **답:** 아니다. commit과 merge는 별개다. page가 query로 읽힐 때, background·purge 작업이나 slow shutdown 등에서 나중에 merge될 수 있다.

> [!question]- `deletes`와 `purges` 옵션은 어떻게 다른가?
> **답:** `deletes`는 secondary index item에 delete mark를 남기는 논리 삭제를 buffer하고, `purges`는 purge가 delete-marked item을 실제 index page에서 제거하는 물리 삭제를 buffer한다. `changes`는 inserts와 deletes를, `all`은 세 종류를 모두 포함한다.

> [!question]- `innodb_change_buffer_max_size=25`는 무엇을 뜻하는가?
> **답:** change buffer가 buffer pool의 최대 25%까지 사용할 수 있다는 상한이다. 시작할 때 25%를 고정 할당한다는 뜻이 아니며, 값을 키우면 data·index page cache와 memory를 경쟁할 수 있다.

> [!question]- `innodb_change_buffering=none`으로 바꾸면 기존 entry도 즉시 없어지는가?
> **답:** 아니다. 새 change buffering만 중지한다. 이미 존재하는 entry는 target page가 읽히거나 background merge가 진행되면서 계속 원래 secondary index page에 합쳐진다.

> [!question]- change buffer가 특히 유리하거나 불리할 수 있는 workload는 무엇인가?
> **답:** buffer pool miss가 많은 non-unique secondary index를 대량 변경하는 I/O-bound workload에서 유리할 가능성이 크다. working set이 memory에 있고 secondary index가 적거나 대부분 UNIQUE인 read-heavy workload에서는 이득이 작고 buffer pool memory 경쟁과 merge 비용이 더 클 수 있다.

> [!question]- WAL에서 "redo를 먼저 쓴다"는 무엇보다 먼저라는 뜻인가?
> **답:** 해당 변경이 담긴 data page를 disk에 쓰기 전에 그 변경을 재현할 redo record가 durable해야 한다는 뜻이다. 매 SQL이 commit될 때 data page와 redo를 모두 즉시 쓴다는 뜻은 아니다.

> [!question]- `COMMIT`이 성공하면 dirty data page도 이미 disk data file에 기록된 것인가?
> **답:** 아니다. 기본 설정 1에서는 commit에 필요한 redo가 write·flush된 것이지, data page는 buffer pool에 dirty 상태로 남아 page cleaner·checkpoint가 나중에 flush할 수 있다.

> [!question]- crash recovery는 redo만 재실행하면 끝나는가?
> **답:** 아니다. 최근 checkpoint 이후의 durable redo를 roll-forward해 page 상태를 맞춘 뒤, crash 때 commit되지 않은 transaction의 변경은 undo로 rollback한다.

> [!question]- `innodb_flush_log_at_trx_commit` 0·1·2의 차이를 write와 flush로 설명하면?
> **답:** 1은 commit마다 write와 disk flush를 모두 한다. 0은 commit에서 둘 다 강제하지 않고 약 N초마다 write·flush한다. 2는 commit마다 write하지만 disk flush는 약 N초마다 한다. N은 `innodb_flush_log_at_timeout`이며 실제 간격은 정확히 보장되지 않는다.

> [!question]- `innodb_flush_log_at_trx_commit=2`는 언제 data를 잃을 수 있는가?
> **답:** commit 시 redo를 OS에 write하므로 mysqld 프로세스만 crash하면 남을 수 있지만, OS crash·전원 상실·storage cache 유실이 발생하면 아직 flush되지 않은 최근 N초 transaction을 잃을 수 있다. 그래서 full ACID 설정은 아니다.

> [!question]- log buffer를 크게 하면 항상 commit이 빨라지는가?
> **답:** 아니다. 하나의 큰 transaction이 많은 redo를 만들어 COMMIT 전 buffer 공간 확보용 disk write가 발생할 때 도움이 될 수 있다. 일반적인 작은 OLTP transaction에서는 큰 값이 공짜 성능을 주지 않으며 global memory를 더 사용한다.

> [!question]- 책의 `innodb_log_file_size × innodb_log_files_in_group`을 MySQL 8.0.44에서도 redo capacity 조정 방법으로 사용해야 하는가?
> **답:** 아니다. 8.0.30+는 `innodb_redo_log_capacity`가 전체 redo capacity를 dynamic하게 관리하며 기존 두 변수는 deprecated되었다. `#innodb_redo`의 ordinary·spare file 약 32개 모델을 구분해야 한다.

> [!question]- redo log archiving은 왜 하며 binary log와 같은가?
> **답:** 물리 backup이 data file을 복사하는 동안 필요한 redo가 순환 file에서 덮어쓰기 전 별도 file에 보존하기 위해서다. backup 일관성을 돕는 물리 redo 스트림이지 replication·PITR용 binary log를 대체하지 않는다.

> [!question]- `ALTER INSTANCE DISABLE INNODB REDO_LOG`를 운영 server의 bulk load에 써도 되는가?
> **답:** 안 된다. 새 instance 초기 data load에만 의도된 기능이다. disabled 상태에서 crash하면 data 유실·instance 손상·시작 거부가 발생할 수 있고 backup·archive와도 충돌하므로 운영에서 성능 tuning 수단으로 사용하지 않는다.

> [!question]- 어댑티브 해시 인덱스(AHI)는 사용자가 `CREATE INDEX`로 만드는 index인가?
> **답:** 아니다. InnoDB가 buffer pool에 있는 index page의 반복 검색 pattern을 관찰해 자동으로 만드는 내부 hash shortcut이다. schema에 나타나지 않고 optimizer가 별도 access path로 선택하는 index도 아니다. 비활성화되면 query는 원래 B+Tree 탐색으로 돌아간다.

> [!question]- AHI는 B+Tree 탐색의 무엇을 줄이는가?
> **답:** 자주 조회되는 `B+Tree index ID + key prefix`를 buffer-pool page·record 위치에 연결해 root와 branch node를 반복해서 내려가는 CPU 비용을 줄인다. target page가 memory에 있을 때의 shortcut이므로 disk에서 page를 읽는 비용을 없애지는 않는다.

> [!question]- AHI가 유리한 workload와 불리한 workload는 무엇인가?
> **답:** working set이 buffer pool에 있고 동일 hot key에 대한 equality lookup이 반복되는 workload에서 유리할 수 있다. disk I/O가 병목이거나 range·`LIKE '%…'`·wide scan이 많거나, 많은 thread가 같은 AHI partition latch를 다투는 workload에서는 이득이 작거나 오히려 느려질 수 있다.

> [!question]- Spring Data JPA나 Hibernate에서 AHI를 켜는 annotation·query hint가 있는가?
> **답:** 없다. AHI는 InnoDB instance 전역의 내부 기능이다. JPA가 생성하는 SQL의 equality 조건·hot-key 집중도·동시성은 AHI가 유효한 workload인지에 간접 영향을 줄 뿐, application query 하나가 AHI 사용을 명령하거나 `EXPLAIN`에서 선택할 수는 없다.

> [!question]- `hash searches/s ÷ (hash + non-hash searches/s)`가 높으면 AHI를 유지하면 되는가?
> **답:** 아니다. 이 비율은 AHI 경유 빈도의 관측치일 뿐 효율이나 latency 개선율이 아니다. 같은 대표 workload에서 warmup 뒤 AHI ON/OFF의 throughput·p95/p99·CPU·buffer-pool miss와 `btr0sea.c` latch wait를 함께 비교해야 한다.

> [!question]- `innodb_adaptive_hash_index`와 `innodb_adaptive_hash_index_parts`의 변경 특성은 어떻게 다른가?
> **답:** 전자는 GLOBAL dynamic이라 실행 중 ON/OFF할 수 있다. OFF하면 AHI를 비우고 query는 B+Tree로 처리되며, ON하면 workload에 따라 다시 만들어진다. 후자는 GLOBAL static이라 server 시작 설정이며 MySQL 8.0 기본은 8, 허용 범위는 1~512다.

> [!question]- MySQL 8.0과 8.4의 AHI 기본값은 같은가?
> **답:** 아니다. MySQL 8.0은 기본 `ON`이고 MySQL 8.4부터 기본 `OFF`다. 따라서 책의 8.0 설명을 다른 major version에 그대로 적용하지 말고 실행 중인 server version과 실제 변수값을 확인한다.

> [!question]- AHI partition을 무조건 늘리면 성능이 좋아지는가?
> **답:** 아니다. partition은 hash table·latch contention을 분산할 수 있지만 workload가 AHI로 이득을 보는지와 별개이고 memory·관리 비용도 있다. 먼저 `btr0sea.c` 관련 latch wait와 CPU·latency로 contention을 확인하고, 변경하려면 restart가 필요하므로 대표 부하 A/B 결과를 근거로 정한다.

## 이어지는 내용

- 다음 시작점: **4.2.13 InnoDB와 MyISAM, MEMORY 스토리지 엔진 비교**
- 4.2.9에서 정리한 undo lifecycle을 5장 transaction·lock과 장기 transaction 운영 실험으로 확장
- 4.2.10 change buffer를 8장 secondary index·write amplification·bulk DML 실험과 연결
- 4.2.11 redo log를 5장 transaction·binary log·replication·backup의 durability 정책과 연결
- 4.2.12 adaptive hash index를 8장 B+Tree 탐색 비용·buffer-pool locality·hot-key contention 실험과 연결
...
- [InnoDB Log Buffer](https://dev.mysql.com/doc/refman/8.0/en/innodb-redo-log-buffer.html)
- [`innodb_flush_log_at_trx_commit`·`innodb_flush_log_at_timeout` 시스템 변수](https://dev.mysql.com/doc/refman/8.0/en/innodb-parameters.html#sysvar_innodb_flush_log_at_trx_commit)
- [`ALTER INSTANCE ... INNODB REDO_LOG`](https://dev.mysql.com/doc/refman/8.0/en/alter-instance.html)
- [Performance Schema `innodb_redo_log_files`](https://dev.mysql.com/doc/refman/8.0/en/performance-schema-innodb-redo-log-files-table.html)
...
- [InnoDB Undo Tablespaces](https://dev.mysql.com/doc/refman/8.0/en/innodb-undo-tablespaces.html)
- [InnoDB Change Buffer](https://dev.mysql.com/doc/refman/8.0/en/innodb-change-buffer.html)
- [Converting Tables from MyISAM to InnoDB · `unique_checks`와 change buffering](https://dev.mysql.com/doc/refman/8.0/en/converting-tables-to-innodb.html)
- [MySQL 8.0 Adaptive Hash Index](https://dev.mysql.com/doc/refman/8.0/en/innodb-adaptive-hash.html)
- [MySQL 8.0 InnoDB system variables · AHI ON/OFF와 partition](https://dev.mysql.com/doc/refman/8.0/en/innodb-parameters.html)
- [MySQL 8.0 InnoDB Standard Monitor · `hash searches/s`](https://dev.mysql.com/doc/refman/8.0/en/innodb-standard-monitor.html)
- [MySQL 8.4 InnoDB system variables · AHI 기본값 변경](https://dev.mysql.com/doc/refman/8.4/en/innodb-parameters.html)
