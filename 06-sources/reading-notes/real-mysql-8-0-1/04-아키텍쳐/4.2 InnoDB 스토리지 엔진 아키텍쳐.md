...
> [!abstract] 이 절의 한 문장
> InnoDB는 clustered index를 데이터의 기준 주소로 삼고, foreign key 검사에는 관련 index와 lock을 사용하며, MVCC에서는 read view와 undo version으로 일반 `SELECT`가 writer를 기다리지 않는 consistent read를 제공한다. 오래된 read view는 undo purge를 지연시키고, locking read·DML 사이의 wait cycle은 deadlock detector가 작은 transaction을 rollback해 끊는다. 고동시성에서 detector를 끈다면 즉시 감지를 포기하고 lock-wait timeout과 application 전체 transaction retry로 비용을 옮긴다.
...
사진의 `buffer pool·change buffer·adaptive hash index·log buffer·background thread·data file·transaction log`은 앞으로 4.2 전체에서 하나씩 확장한다. 현재까지의 중심은 **row를 어떤 index로 찾는가**, **동시에 변경 중인 row를 어떤 version으로 읽는가**, **old version을 언제 purge할 수 있는가**, **lock wait cycle을 어떻게 끊는가**다.
