...
> [!abstract] 이 절의 한 문장
> InnoDB는 clustered index와 MVCC·lock으로 row를 관리하고, buffer pool·redo·page cleaner로 읽기 cache와 지연 쓰기를 구현한다. Doublewrite는 torn page를 복구하고, undo log는 rollback과 consistent read에 필요한 이전 version을 보존한다. 오래된 read view가 사라져야 purge가 undo를 재사용할 수 있으며, 필요하면 undo tablespace를 물리적으로 truncate한다.
...
사진의 `buffer pool·change buffer·adaptive hash index·log buffer·background thread·data file·transaction log`은 4.2 전체에서 하나씩 확장한다. 현재까지의 중심은 **row와 version 선택**, **lock wait cycle**, **crash recovery**, **memory page와 redo 공간의 순환**, **flush 속도 제어**, **재시작 warmup**, **torn page 방어**, **undo의 생성·보존·purge·truncate 생명주기**다.
...
- 다음 시작점: **4.2.10 체인지 버퍼(Change Buffer)**
