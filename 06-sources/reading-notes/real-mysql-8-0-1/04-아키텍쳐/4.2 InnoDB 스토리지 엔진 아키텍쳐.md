...
> [!abstract] 이 절의 한 문장
> InnoDB는 clustered index와 MVCC·lock으로 row를 관리하고, buffer pool·redo·page cleaner로 읽기 cache와 지연 쓰기를 구현한다. Doublewrite는 torn page를 복구하고, undo log는 rollback과 consistent read에 필요한 이전 version을 보존한다. 오래된 read view가 사라져야 purge가 undo를 재사용할 수 있다. Change buffer는 buffer pool에 없는 eligible secondary index page의 변경을 기록해 random I/O를 뒤로 미루고, page가 읽힐 때 merge한다.
