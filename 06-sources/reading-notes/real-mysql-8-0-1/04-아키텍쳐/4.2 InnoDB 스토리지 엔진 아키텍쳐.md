...
> [!abstract] 이 절의 한 문장
> InnoDB는 clustered index를 데이터의 기준 주소로 삼고, read view·undo와 lock으로 동시성을 제어한다. 정상 crash recovery는 redo를 재실행하고 미완료 transaction을 undo하지만, 손상 때문에 시작하지 못할 때의 `innodb_force_recovery`는 수리가 아니라 데이터를 덤프하기 위한 비상 기동 모드다. 평상시 읽기와 쓰기의 중심에는 buffer pool이 있으며, LRU·free·flush list가 page를 관리하고 redo log의 checkpoint가 dirty page를 언제까지 disk로 내려야 하는지 제한한다.
...
사진의 `buffer pool·change buffer·adaptive hash index·log buffer·background thread·data file·transaction log`은 4.2 전체에서 하나씩 확장한다. 현재까지의 중심은 **row를 어떤 index로 찾는가**, **동시에 변경 중인 row를 어떤 version으로 읽는가**, **lock wait cycle을 어떻게 끊는가**, **crash 뒤 무엇을 재실행·rollback하는가**, **memory page와 redo 공간을 어떻게 순환시키는가**다.
...
각 instance는 자체 LRU·free·flush list와 hash 구조를 가진다. MySQL 8.0 공식 기본값은 1이고, `innodb_buffer_pool_size >= 1GB`일 때만 여러 instance가 유효하다. 여러 instance를 쓴다면 instance당 최소 1GB가 되도록 권장한다. 따라서 책의 “기본 8개·5GB당 1개”는 특정 version·대형 server용 heuristic이며 현재 설치 version의 default와 workload contention을 먼저 확인한다.
...
- 다음 시작점: **4.2.7.4 버퍼 풀 플러시(Buffer Pool Flush)**
- 4.2.7.4에서 flush list·LRU list flush와 adaptive flushing이 checkpoint age를 어떻게 낮추는지 확장
- 4.2.7.5에서 buffer pool 상태 backup·restore, 4.2.7.6에서 적재 page 관측으로 연결
- 4.2.8 Double Write Buffer에서 torn page와 redo만으로 복구할 수 없는 경우를 연결
